...
PATCH_ID = "FELFEL-UXUI-REFERENCE-DESIGN-V4"
TARGET = "client/src/pages/FelfelPage.tsx"
EXPECTED_TARGET_BLOB = "cd541f5ff161fad39ca0e98b0791917bca4243ac"
ALLOWED_BRANCHES = {"main", "feature/darwish-phase-a-data-foundation"}
...
if branch not in ALLOWED_BRANCHES:
    raise SystemExit(f"{PATCH_ID} refuses branch {branch!r}; allowed: {sorted(ALLOWED_BRANCHES)}")
...
if tracked_blob != EXPECTED_TARGET_BLOB:
    raise SystemExit(
        f"Target blob mismatch for {TARGET}: expected {EXPECTED_TARGET_BLOB}, found {tracked_blob}. "
        "This patch is intentionally file-baseline-safe and will not guess across unrelated source states."
    )
...
text = replace_once(text, 'data-felfel-uxui="premium-v1"', 'data-felfel-uxui="reference-v4"', "design marker")
text = replace_once(
    text,
...
if status_after.strip() != expected:
    raise SystemExit(f"Unexpected worktree after patch. Expected {expected!r}; found:\n{status_after}")

updated = path.read_text(encoding="utf-8")
for marker in (
    'data-felfel-uxui="reference-v4"',
    'object-[50%_18%]',
    'border-orange-500',
