...
PATCH_NAME="ZAGHLOUL_V5R2_TSC_BASELINE_CORRECTIVE"
...
# The router is known stale only if nothing else imports/references its exported symbol/path.
set +e
REFERENCES="$(cd "$TARGET" && grep -RInE 'zaghloulV5Router|routes/zaghloul-v5|zaghloul-v5/router' server --exclude='router.ts' 2>/dev/null)"
REF_RC=$?
set -e
...
# Runtime-neutral correction: remove unused duplicate router and quarantine only this bridge's legacy type diagnostics.
