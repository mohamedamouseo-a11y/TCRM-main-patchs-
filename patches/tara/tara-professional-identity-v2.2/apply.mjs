...
/**
 * TCRM — Tara Professional Identity V2.2
 * Corrective UX/UI patch for the production avatar routing issue.
 * The portrait is moved into client/src/assets and imported by Vite.
 * No API, DB, route, permission, or business-logic changes.
 */
...
const avatarB64Path = path.resolve(patchDir, '../tara-professional-identity-v2/tara-avatar-320.jpg.b64');
const avatarTarget = path.resolve(cwd, 'client/src/assets/ai-staff/tara-avatar.jpg');

function fail(message, code = 1) {
  console.error(`[tara-identity-v2.2] ${message}`);
  process.exit(code);
}
function info(message) {
  console.log(`[tara-identity-v2.2] ${message}`);
}
...
function isPatched(input) {
  return input.includes('import taraAvatar from "@/assets/ai-staff/tara-avatar.jpg";')
    && input.includes('function getTaraIdentity(isRTL: boolean)')
...
if (mode === '--check') {
  ...
  info(isPatched(source) ? 'already patched' : 'ready to apply V2.2 bundled-avatar fix');
  process.exit(0);
}
...
if (mode === '--apply') {
  ...
  info('applied Tara Identity V2.2 with Vite-bundled avatar asset');
  process.exit(0);
}

if (mode === '--verify') {
  if (!isPatched(source)) fail('expected Tara V2.2 identity source markers are missing');
  if (!fs.existsSync(avatarTarget)) fail(`bundled avatar was not installed: ${avatarTarget}`);
