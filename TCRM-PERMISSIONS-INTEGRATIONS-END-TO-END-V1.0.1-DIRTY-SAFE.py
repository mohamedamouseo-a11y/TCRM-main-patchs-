...
VERSION = "V1.0.1"
WORKFLOW_ID = "TCRM-PERMISSIONS-INTEGRATIONS-END-TO-END-V1.0.1-DIRTY-SAFE"
BASELINE = "e0d9096b4043c90321310a8344e538f3e15ba555"
ROOT = Path.cwd()
ORIGINAL_PATCH_URL = "https://raw.githubusercontent.com/mohamedamouseo-a11y/TCRM-main-patchs-/main/TCRM-PERMISSIONS-INTEGRATIONS-END-TO-END-V1.0.0.py"

PREEXISTING_DIRTY = {
    "client/src/pages/LeadProfile.tsx",
    "client/src/lead-profile-premium-v1.css",
}
...
# validated Lead Profile Premium V1 files may be dirty before Integrations work.
...
# Reuse the already-reviewed V1.0.0 implementation. Only its safety bookkeeping
# is adapted in-memory so it can coexist with the exact preexisting Lead Profile
# dirty set. No product implementation logic is rewritten here.
try:
...
namespace = {"__name__": "__main__", "__file__": "/tmp/TCRM-PERMISSIONS-INTEGRATIONS-END-TO-END-V1.0.0.py"}
try:
...
# Byte-for-byte preservation check for the unrelated Lead Profile Premium V1 work.
...
expected_final = PREEXISTING_DIRTY | INTEGRATIONS_EXPECTED
if final_dirty != expected_final:
    fail(f"RECOVERY_FINAL_DIRTY_SET_MISMATCH:{sorted(final_dirty)}")

tracked_integrations = sorted(INTEGRATIONS_EXPECTED - {"server/security/integrationsPermissionFinal.test.ts"})
run("git", "diff", "--check", "--", *tracked_integrations)
...
print("INTEGRATIONS_FILES_EXPECTED=" + ",".join(sorted(INTEGRATIONS_EXPECTED)))
