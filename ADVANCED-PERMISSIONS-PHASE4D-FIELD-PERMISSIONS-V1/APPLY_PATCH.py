...
def replace_once_after(text: str, marker: str, old: str, new: str, label: str) -> str:
    ...
    if marker_count != 1:
        raise SystemExit(f"Marker mismatch for {label}: expected exactly 1 marker, got {marker_count}. No files changed.")
    before, after = text.split(marker, 1)
    count = after.count(old)
    if count != 1:
        raise SystemExit(f"Anchor mismatch after marker for {label}: expected exactly 1 match, got {count}. No files changed.")
