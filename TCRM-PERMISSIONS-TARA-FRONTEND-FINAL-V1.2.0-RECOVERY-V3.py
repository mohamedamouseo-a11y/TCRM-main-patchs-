...
VERSION = "V1.2.0"
WORKFLOW_ID = "TCRM-PERMISSIONS-TARA-FRONTEND-FINAL-V1.2.0-RECOVERY-V3"
PREVIOUS_VERSION = "V1.0.1"
PREVIOUS_WORKFLOW_ID = "TCRM-PERMISSIONS-TARA-API-ENFORCEMENT-V1.0.1-RECOVERY"
RECOVERY_FROM_WORKFLOW_ID = "TCRM-PERMISSIONS-TARA-FRONTEND-FINAL-V1.2.0-RECOVERY-V2"
...
def proc(*args: str) -> subprocess.CompletedProcess[str]:
...
def assert_once(text: str, marker: str, label: str) -> None:
    ...
    if count != 1:
        fail(f"STATE_MISMATCH:{label}:count={count}")


def dirty_paths() -> set[str]:
    """Return unstaged tracked changes + untracked files without parsing status columns."""
...
def verify_partial_three() -> None:
    ...
    count = page.count(operations_mount)
    if count != 2:
        fail(f"STATE_MISMATCH:tara-page-operations-mount:count={count}")
    ...
    operations = read("client/src/components/tara/TaraModeratorOperations.tsx")
    for marker in [
        'allowOperate = true',
        ...
        assert_once(operations, marker, f"tara-operations:{marker}")
    ...
    for name in [
        ...
        assert_once(operations, f"const {name}Raw = trpc.", f"tara-operations-raw:{name}")
        assert_once(
            operations,
...
def build_workspace_from_clean_head() -> str:
    rel = "client/src/components/tara/TaraModeratorWorkspace.tsx"
    p = proc("git", "show", f"HEAD:{rel}")
    if p.returncode != 0:
        fail(f"COMMAND_FAILED:git show HEAD:{rel}:{p.stderr.strip()}")
    s = p.stdout
    ...
    for name, route in routes.items():
        s = rename_route_mutation(s, name, route, "workspace-moderate")
    ...
    s = replace_once(
        ...
        "workspace-evolution-prop",
    )
    s = rename_route_mutation(s, "statusM", "tara.setConversationStatus", "workspace-evolution-status")
    s = insert_after_route_line(
        s,
        "statusM",
        "tara.setConversationStatus",
        '    const statusM = guardTaraMutation(statusMRaw, allowOperate, "tara.operate");\n',
        "workspace-evolution-status-wrapper",
    )
    return s
...
def verify_workspace_final(workspace: str) -> None:
    for marker in [
        'function guardTaraMutation(',
        'allowModerate = true',
        'allowOperate = true',
        'data-tara-read-only="moderate"',
        'const generateMRaw = trpc.tara.moderation.generateReply.useMutation',
        'const sendMRaw = trpc.tara.moderation.sendReply.useMutation',
        'const retryMRaw = trpc.tara.moderation.retryReply.useMutation',
        'const updateMRaw = trpc.tara.moderation.updateConversation.useMutation',
        'const noteMRaw = trpc.tara.moderation.addNote.useMutation',
...
def verify_final_test(text: str) -> None:
    for marker in [
        'describe("Tara permissions final V1.2.0"',
        'tara.futureUnknownOperation',
        'tara.moderation.sendReply',
        'data-tara-read-only="operate"',
        'data-tara-read-only="moderate"',
        'trpc.waGateway.sendText.useMutation',
        'TaraProviderSettingsPanel',
        'TaraVoiceSettingsPanel',
        'TaraSocialChannelsPanel',
    ]:
        if marker not in text:
            fail(f"FINAL_TEST_STATE_MISMATCH:missing:{marker}")
...
if current_dirty == PARTIAL_EXPECTED:
    input_state = "PARTIAL_3"
elif current_dirty == FINAL_EXPECTED:
    input_state = "FINAL_5_FROM_V2"
else:
    fail(f"RECOVERY_INPUT_SET_MISMATCH:{sorted(current_dirty)}")

verify_partial_three()
expected_workspace = build_workspace_from_clean_head()
workspace_rel = "client/src/components/tara/TaraModeratorWorkspace.tsx"
final_test_rel = "server/security/taraFinalPermission.test.ts"

if input_state == "PARTIAL_3":
    # Complete only the two files that were not written by the original failed patch.
    write(workspace_rel, expected_workspace)
    if (ROOT / final_test_rel).exists():
        fail("UNEXPECTED_FINAL_TEST_BEFORE_WRITE")
    write(final_test_rel, FINAL_TEST)
else:
    # V2 reaches its final dirty-set check only after writing these two files.
    # Do not rewrite them; verify the state that is already present.
    workspace_now = read(workspace_rel)
    if workspace_now != expected_workspace:
        fail("V2_WORKSPACE_CONTENT_MISMATCH")
    verify_final_test(read(final_test_rel))
...
workspace_now = read(workspace_rel)
if workspace_now != expected_workspace:
    fail("FINAL_WORKSPACE_EXACT_MISMATCH")
verify_workspace_final(workspace_now)
verify_final_test(read(final_test_rel))
