...
if UI_MARKER not in ui:
    new_ui = replace_once(
        new_ui,
        'import { CheckCircle2, Copy, Plus, Save, Search, Shield, Trash2, Users, UserCog } from "lucide-react";',
        'import { CheckCircle2, Copy, Plus, Save, Search, Shield, Trash2, Users, UserCog, TestTube2 } from "lucide-react";',
        "tester icon import",
    )
    new_ui = replace_once(
        new_ui,
    ...
    tester_tab = '          <button onClick={() => setTab("tester")} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition ${tab === "tester" ? "bg-background shadow-sm" : "text-muted-foreground hover:text-foreground"}`}><TestTube2 className="h-4 w-4" />{isRTL ? "اختبار الصلاحيات" : "Permission Tester"}</button>\n'
    new_ui = replace_once(
        new_ui,
        override_tab,
...
# Only write after every required anchor has been verified.
