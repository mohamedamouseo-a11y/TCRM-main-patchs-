...
PATCH_ID = "FELFEL-UXUI-REFERENCE-DESIGN-V5"
...
EXPECTED_CURRENT_BLOB = "3d1c2ebc9212d59f34d0ffad78040cf1b334e55c"
...
def require_only_mautic_untracked(lines: list[str], phase: str) -> None:
    unexpected = [line for line in lines if not line.startswith("?? external/mautic/")]
    if unexpected:
        raise SystemExit(
            f"{phase}: unexpected worktree entries outside external/mautic:\n" + "\n".join(unexpected[:50])
        )
...
if branch != "main":
    raise SystemExit(f"{PATCH_ID} requires branch main; found {branch!r}")

head = run("git", "rev-parse", "HEAD")
if head != BASELINE_HEAD:
    raise SystemExit(f"{PATCH_ID} requires HEAD {BASELINE_HEAD}; found {head}")

# Tracked tree/index must be clean. Untracked Mautic runtime material is explicitly tolerated.
if run("git", "diff", "--name-only"):
    raise SystemExit("Tracked worktree changes exist; refusing UI patch")
if run("git", "diff", "--cached", "--name-only"):
    raise SystemExit("Staged changes exist; refusing UI patch")
status_before = porcelain_lines()
require_only_mautic_untracked(status_before, "PRECHECK")
mautic_count_before = sum(1 for line in status_before if line.startswith("?? external/mautic/"))
...
tracked_blob = run("git", "rev-parse", f"HEAD:{TARGET}")
if tracked_blob != EXPECTED_CURRENT_BLOB:
    raise SystemExit(f"Current Felfel page blob mismatch: expected {EXPECTED_CURRENT_BLOB}, found {tracked_blob}")

# The previously validated premium UI commit must still exist locally and be preserved exactly.
source_page_blob = run("git", "rev-parse", f"{SOURCE_COMMIT}:{TARGET}")
source_avatar_blob = run("git", "rev-parse", f"{SOURCE_COMMIT}:{AVATAR}")
if source_page_blob != EXPECTED_SOURCE_BLOB:
    raise SystemExit(f"Source Felfel page blob mismatch: expected {EXPECTED_SOURCE_BLOB}, found {source_page_blob}")
if source_avatar_blob != EXPECTED_AVATAR_BLOB:
    raise SystemExit(f"Source avatar blob mismatch: expected {EXPECTED_AVATAR_BLOB}, found {source_avatar_blob}")

# If an avatar already exists outside Git tracking, it must be the exact validated blob.
if avatar.exists():
    existing_avatar_blob = run("git", "hash-object", AVATAR)
    if existing_avatar_blob != EXPECTED_AVATAR_BLOB:
        raise SystemExit(
            f"Existing avatar differs from validated asset: expected {EXPECTED_AVATAR_BLOB}, found {existing_avatar_blob}"
        )

# Restore the validated premium UX shell and avatar from the already-local preserved commit.
page.write_bytes(run_bytes("git", "show", f"{SOURCE_COMMIT}:{TARGET}"))
avatar.parent.mkdir(parents=True, exist_ok=True)
if not avatar.exists():
    avatar.write_bytes(run_bytes("git", "show", f"{SOURCE_COMMIT}:{AVATAR}"))
...
for marker in (
    ...
    if marker not in text:
        raise SystemExit(f"Expected premium source marker missing after local restore: {marker}")
...
text = replace_once(text, 'data-felfel-uxui="premium-v1"', 'data-felfel-uxui="reference-v5"', "design marker")
text = replace_once(
    text,
...
# The only tracked diff is the Felfel page. The avatar is either exact ignored/local content or a new exact file.
tracked_after = [p for p in run("git", "diff", "--name-only").splitlines() if p.strip()]
if tracked_after != [TARGET]:
    raise SystemExit(f"Unexpected tracked diff after patch: {tracked_after}")
if run("git", "diff", "--cached", "--name-only"):
    raise SystemExit("Unexpected staged changes after patch")

status_after = porcelain_lines()
allowed_exact = {f" M {TARGET}", f"?? {AVATAR}"}
unexpected_after = [
    line for line in status_after
    if line not in allowed_exact and not line.startswith("?? external/mautic/")
]
if unexpected_after:
    raise SystemExit("Unexpected worktree after patch:\n" + "\n".join(unexpected_after[:50]))
mautic_count_after = sum(1 for line in status_after if line.startswith("?? external/mautic/"))
if mautic_count_after != mautic_count_before:
    raise SystemExit(
        f"external/mautic status count changed unexpectedly: before={mautic_count_before}, after={mautic_count_after}"
    )
...
final_avatar_blob = run("git", "hash-object", AVATAR)
if final_avatar_blob != EXPECTED_AVATAR_BLOB:
    raise SystemExit(f"Final avatar blob mismatch: expected {EXPECTED_AVATAR_BLOB}, found {final_avatar_blob}")

print(f"{PATCH_ID} applied")
print(f"BRANCH={branch}")
print(f"HEAD={head}")
print(f"TARGET_BLOB_BEFORE={tracked_blob}")
print(f"PREMIUM_SOURCE_COMMIT={SOURCE_COMMIT}")
print(f"PREMIUM_SOURCE_BLOB={source_page_blob}")
print(f"AVATAR_BLOB={final_avatar_blob}")
print(f"MAUTIC_UNTRACKED_COUNT_PRESERVED={mautic_count_after}")
...
print("HERO_REFERENCE_LAYOUT=YES")
print("METRIC_CARDS_REFERENCE_LAYOUT=YES")
print("TAB_ORANGE_ACTIVE_EDGE=YES")
print("NEW_MEETING_REFERENCE_LAYOUT=YES")
print("LIVE_STATUS_REFERENCE_LAYOUT=YES")
print("SUMMARY_CARDS_REFERENCE_LAYOUT=YES")
print("SERVICE_CAPABILITIES_REFERENCE_LAYOUT=YES")
