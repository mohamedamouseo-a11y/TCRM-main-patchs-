...
VERSION = "V1.2.0"
WORKFLOW_ID = "TCRM-PERMISSIONS-TARA-FRONTEND-FINAL-V1.2.0-RECOVERY-V2"
...
# Verify the three files written before the previous anchor-drift failure.
...
# This mount occurs intentionally twice: Moderator operations view + non-manage fallback.
operations_mount = '<TaraModeratorOperations profile={profileQ.data} allowOperate={canOperate}/>'
operations_mount_count = page.count(operations_mount)
if operations_mount_count != 2:
    fail(f"PARTIAL_STATE_MISMATCH:tara-page-operations-mount:count={operations_mount_count}")
...
# Complete TaraModeratorWorkspace with route-qualified anchors. The file has two
# variables named sendM, so variable-name-only matching is intentionally forbidden.
...
# Final fail-closed dirty set.
status_lines = [line for line in run("git", "status", "--short", "--untracked-files=all").splitlines() if line]
final_paths = {line[3:] for line in status_lines if len(line) >= 4}
