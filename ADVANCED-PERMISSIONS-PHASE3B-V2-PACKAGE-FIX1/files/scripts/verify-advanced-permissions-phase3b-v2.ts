...
for (const role of ["ServiceAdvisor", "PartsAgent", "CrmFollowUp"]) {
  if (serverActive.includes(`\"${role}\"`) || clientActive.includes(`\"${role}\"`)) throw new Error(`Automotive-only role still active/selectable: ${role}`);
}

const adminRouter = must("server/permissionsAdminRouter.ts", "ADVANCED_PERMISSIONS_PHASE3B_V2_USER_OVERRIDES");
for (const key of ["listUsersForPermissions", "getUserPermissionProfile", "replaceUserOverrides"]) if (!adminRouter.includes(key)) throw new Error(`permissionsAdminRouter missing ${key}`);

const overrideService = must("server/security/permissionUserOverrideAdmin.ts", "replacePermissionUserOverrides");
for (const table of ["user_permission_overrides", "permission_audit_logs"]) if (!overrideService.includes(table)) throw new Error(`override service missing ${table}`);
...
for (const needle of ["User Overrides", "listUsersForPermissions", "replaceUserOverrides"]) if (!ui.includes(needle)) throw new Error(`RolesPermissions UI missing ${needle}`);

console.log(JSON.stringify({
  ok: true,
  phase: "3B-v2-package-fix1",
  verified: [
  ...
  productionCodeChangedByFix1: false
}, null, 2));
